/**
 * Created by quanquan.sun on 2017/4/21 0021.
 */
...
/*var baseUrl = '//http://m.iqiyi.com/vip/';
var pageUrls = [];
...
var pageUrls = [];
pageUrls.push("http://m.iqiyi.com/vip/");
    ...
    pageUrls.forEach(function (page) {
        ...
        superagent.get(page).end(function (err, sres) {
            ...
            for(var i=0;i<moreLink.length;i++){
                console.log(moreLink.eq(i).attr('class'));
                if(moreLink.eq(i).attr('class') == 'site-piclist_pic_link') {
                    authorUrls.push(moreLink.eq(i).attr('href'));
                }
            }
            /*$('.m_link').each(function (i, e) {
                console.log(e);
                var u = $('.m_link', e).attr('href');
